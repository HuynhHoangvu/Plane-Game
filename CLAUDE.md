...
Ban đầu dùng Web Speech API (`speechSynthesis`) nhưng phát hiện: nhiều máy không cài giọng đọc tiếng Đức ở tầng OS, khiến trình duyệt tự fallback sang giọng tiếng Anh mặc định mà không báo lỗi — web app không có quyền "cài" voice mới cho OS, đây là giới hạn cứng của Web Speech API. Giải pháp: chuyển hẳn sang engine TTS chạy 100% trong trình duyệt (WASM/asm.js), không phụ thuộc voice cài sẵn.
- **Thư viện**: `mespeak` (JS port của eSpeak, formant synthesis — giọng hơi "robot" nhưng nhẹ, luôn đúng ngôn ngữ, chạy offline hoàn toàn sau khi tải trang). **License GPL** — đã được user đồng ý dùng cho dự án cá nhân này; nếu sau này dự án chuyển hướng thương mại/đóng nguồn cần xem lại license này.
...
- `lib/game/offlineTts.ts`: `primeOfflineTts(lang)` gọi khi vào game để tải trước engine+config+voice (không chặn UI); `speakOffline(text, lang)` gọi mỗi khi cần đọc to, tự chờ engine sẵn sàng, lỗi thì im lặng bỏ qua (không phá gameplay).
- Engine được code-split qua `import()` động (chunk riêng ~880KB sau khi Turbopack xử lý), chỉ tải khi thực sự cần, không nằm trong bundle chính.
